...
What makes a 'better intermediate representation'? To me, it is the following:
...
What do the above look like for different approaches? We can use the following  from the TAG dataset as an example.
...
Additionally, when we execute the `LLMMap` function, it will handle the `DISTINCT` logic for us, making sure we don't ask the language model for the population of Los Angeles more than once.
...
Note: one LOTUS program ([`pipeline_38()`](https://github.com/TAG-Research/TAG-Bench/blob/main/tag/hand_written.py#L468)) took 58 seconds to run. To me, it looks like an incorrect program (the question mentions Lewis Hamilton, but the code calls the LM for **all** drivers), and I've removed it from the latency comparisons below.

|          | Model                           | Hardware          | Accuracy (↑) | Execution Time (s) (↓) |
|----------|---------------------------------|-------------------|--------------|------------------------|
| LOTUS    | Llama-3.1-70b-instruct          | 8 A100, 80GB      | 0.55         | 3.0475                 |
| BlendSQL | Meta-Llama-3.1-8B-Instruct.Q6_K | 1 RTX 5080, 16GB  | 0.55         | 0.935                  |
